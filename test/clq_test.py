...
class TestFLGeneration:
    ...
    def test_clq_i_1(self, capfd):
        json_comp(self.parser, capfd, 'clq_i_1.json', ['-cd', '1', '-tl', '-rs', '0', 'clq'])

    #../dwig.py -cd 2 -tl -rs 0 clq > data/clq_i_2.json
    def test_clq_i_2(self, capfd):
        json_comp(self.parser, capfd, 'clq_i_2.json', ['-cd', '2', '-tl', '-rs', '0', 'clq'])

    #../dwig.py -cd 3 -tl -rs 0 clq > data/clq_i_3.json
    def test_clq_i_3(self, capfd):
        json_comp(self.parser, capfd, 'clq_i_3.json', ['-cd', '3', '-tl', '-rs', '0', 'clq'])
